...
CREATE VIEW predictdata AS
	...
	(
		SELECT 
			datetime(current_timestamp, 'localtime') as now,
			STRFTIME("%m",datetime(current_timestamp, 'localtime')) as now_m,
			STRFTIME("%j",datetime(current_timestamp, 'localtime')) as now_j,
			STRFTIME("%H",datetime(current_timestamp, 'localtime')) as now_h
	)
    SELECT
        NULL AS Temperature,
        SIN(2 * PI() * CAST(t.now_m AS REAL) / 12) AS month,
        SIN(2 * PI() * CAST(t.now_j AS REAL) / 366) AS day,
        SIN(2 * PI() * CAST(t.now_H AS REAL) / 24) AS hour,
...
--    INNER JOIN meteodata AS currentMeteo
--        ON STRFTIME('%Y-%m-%d %H:00:00', t.now) = STRFTIME('%Y-%m-%d %H:00:00', currentMeteo.Timestamp)
--    INNER JOIN meteodata AS back1Hour
--        ON STRFTIME('%Y-%m-%d %H:00:00', t.now, '-1 hour') = STRFTIME('%Y-%m-%d %H:00:00', back1Hour.Timestamp)
--    INNER JOIN meteodata AS back2Hour
--        ON STRFTIME('%Y-%m-%d %H:00:00', t.now, '-2 hour') = STRFTIME('%Y-%m-%d %H:00:00', back2Hour.Timestamp)
--    INNER JOIN meteodata AS back3Hour
--        ON STRFTIME('%Y-%m-%d %H:00:00', t.now, '-3 hour') = STRFTIME('%Y-%m-%d %H:00:00', back3Hour.Timestamp)
--    INNER JOIN meteodata AS back4Hour
--        ON STRFTIME('%Y-%m-%d %H:00:00', t.now, '-4 hour') = STRFTIME('%Y-%m-%d %H:00:00', back4Hour.Timestamp)
--    INNER JOIN meteodata AS back5Hour
--        ON STRFTIME('%Y-%m-%d %H:00:00', t.now, '-5 hour') = STRFTIME('%Y-%m-%d %H:00:00', back5Hour.Timestamp)
--    INNER JOIN meteodata AS back6Hour
--        ON STRFTIME('%Y-%m-%d %H:00:00', t.now, '-6 hour') = STRFTIME('%Y-%m-%d %H:00:00', back6Hour.Timestamp)
--    INNER JOIN meteodata AS nextHour
--        ON STRFTIME('%Y-%m-%d %H:00:00', t.now, '+1 hour') = STRFTIME('%Y-%m-%d %H:00:00', nextHour.Timestamp)
